...
de tal forma que se possa utilizar a regra \ensuremath{\Conid{Fokkinga}}.
...
%
     \ensuremath{\begin{lcbr}(\Varid{q}\;\Varid{d})\comp \underline{\mathrm{0}}\mathrel{=}\underline{\mathrm{0}}\\(\Varid{q}\;\Varid{d})\comp \succ \mathrel{=}\Varid{add}\;\conj{\Varid{q}\;\Varid{d}}{(\equiv \mathrm{0})\comp (\Varid{cd})\to \underline{\mathrm{1}},\underline{\mathrm{0}}}\end{lcbr}}
%
\just\equiv{ (27), def inNat }
%
     \ensuremath{(\Varid{q}\;\Varid{d})\comp \mathsf{in}\mathrel{=}\alt{\underline{\mathrm{0}}}{\Varid{add}\;\conj{\Varid{q}\;\Varid{d}}{(\equiv \mathrm{0})\comp (\Varid{cd})\to \underline{\mathrm{1}},\underline{\mathrm{0}}}}}
%
\just\equiv{ (3), (32), (7) }
%
     \ensuremath{(\Varid{q}\;\Varid{d})\comp \mathsf{in}\mathrel{=}\alt{\underline{\mathrm{0}}}{\Varid{add}\;(\conj{\Varid{q}\;\Varid{d}}{(\equiv \mathrm{0})\to \underline{\mathrm{1}},\underline{\mathrm{0}}}\comp \p2\comp \conj{\Varid{r}\;\Varid{d}}{\Varid{c}\;\Varid{d}})}}
%
\just\equiv{ (11), (1), (22) }
%
     \ensuremath{(\Varid{q}\;\Varid{d})\comp \mathsf{in}\mathrel{=}\alt{\underline{\mathrm{0}}}{\Varid{add}\;(\conj{\Varid{q}\;\Varid{d}}{(\equiv \mathrm{0})\to \underline{\mathrm{1}},\underline{\mathrm{0}}}\comp \p2)}\comp (\Varid{id}\mathbin{+}\conj{\Varid{qd}}{\conj{\Varid{rd}}{\Varid{cd}}})}
\qed
\end{eqnarray*}
...
\\
\>[B]{}\Varid{alice}\comp \Conid{Leaf}\mathrel{=}\Varid{id}{}\<[E]%
\\
\>[B]{}\Varid{alice}\comp \Conid{Fork}\mathrel{=}\uncurry{\Varid{max}}\comp (\Varid{bob}\times\Varid{bob}){}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{bob}\mathbin{::}\Conid{Ord}\;\Varid{c}\Rightarrow {\LTree}\;\Varid{c}\to \Varid{c}{}\<[E]%
\\
\>[B]{}\Varid{bob}\comp \Conid{Leaf}\mathrel{=}\Varid{id}{}\<[E]%
\\
\>[B]{}\Varid{bob}\comp \Conid{Fork}\mathrel{=}\uncurry{\Varid{min}}\comp (\Varid{alice}\times\Varid{alice}){}\<[E]%
\\[\blanklineskip]%
...
A função \ensuremath{\Varid{propagate}} deve aplicar \ensuremath{\Varid{f}\mathbin{::}\Conid{Monad}\;\Varid{m}\Rightarrow (\Varid{t}\to \Varid{m}\;\Varid{a})} a todos os elementos da lista de entrada, tal como um \ensuremath{\map }, 
enquanto coleta todos os resultados monádicos, de forma tal que se agrupem numa única estrutura 
monádica aplicada a uma lista, ao invés de uma lista de estruturas do mesmo tipo de saída de \ensuremath{\Varid{f}}.
...
uma função que obtenha \ensuremath{\Conid{M}\;\mathit B\mathbin{\uparrow}\{\mskip1.5mu \mathbin{*}\mskip1.5mu\}} a partir de \ensuremath{\mathrm{1}\mathbin{+}\Conid{A}\times\Conid{M}\;\mathit B\mathbin{\uparrow}\{\mskip1.5mu \mathbin{*}\mskip1.5mu\}}. 
Por um lado, a função \ensuremath{\Varid{f}} obtém \ensuremath{\Conid{M}\;\mathit B} a partir do elemento da cabeça da lista \ensuremath{\Conid{A}}, chegando então a \ensuremath{\mathrm{1}\mathbin{+}\Conid{M}\;\mathit B\times\Conid{M}\;\mathit B\mathbin{\uparrow}\{\mskip1.5mu \mathbin{*}\mskip1.5mu\}}.
